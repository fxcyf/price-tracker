...
from app.scrapers.dispatcher import save_domain_cookies
...
async def import_cookies(domain: str, body: CookieImportRequest, db: DB):
    ...
    curl_domain = parsed["domain"]
    if curl_domain and curl_domain != domain:
        raise HTTPException(
            status_code=400,
            detail=f"curl URL domain ({curl_domain}) does not match path domain ({domain})",
        )

    rule = await save_domain_cookies(db, domain, parsed["cookies"])
...
async def get_cookie_status(domain: str, db: DB):
    ...
    result = await db.execute(
        select(DomainRule).where(DomainRule.domain == domain)
    )
    rule = result.scalar_one_or_none()
    if not rule:
        return CookieStatusOut(domain=domain, status=CookieStatus.NONE, cookie_count=None, updated_at=None)
